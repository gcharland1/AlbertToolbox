...
class ImageReader:
    ...
    def find_image_grid(self):
        ...
        h_grid, v_grid = self.get_grid_dimensions(lines, True)
    ...
    def detect_lines(self):
        ...
        if self.save_intermediate:
            file_name = self.work_path + self.image_file + '_edges' + self.extention
            cv2.imwrite(file_name, edges)
            print(f'Edges file saved to {self.work_path}')

        max_line_gap = 3
        minLineLength=int(0.5*min([self.width, self.height]))
        lines = cv2.HoughLinesP(image=thresh,
                                rho=1,
                                theta=np.pi/180,
                                threshold=100,
                                lines=np.array([]),
